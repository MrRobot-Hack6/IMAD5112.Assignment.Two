...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val questions = arrayOf( //(W3Schools, n.d.)
        ...
        val correctAnswers = intArrayOf(1, 0, 0, 1, 1, 1)//(W3Schools, n.d.)
        ...
        btnNext.setOnClickListener {
            ...
            if (currentQuestion < questions.size - 1) {
                ...
                intent.putExtra("Score", score) //(GeeksforGeeks, 2024)
                intent.putExtra("total", questions.size) //(GeeksforGeeks, 2024)
                // Sending as a primitive intArray ensures it is received correctly
                intent.putExtra("Answers", correctAnswers) //(GeeksforGeeks, 2024)
                startActivity(intent) //(GeeksforGeeks, 2024)
                finish() //(GeeksforGeeks, 2024)
            }
        }
        // Setting onClickListener for the back button to navigate back to the WelcomeScreen
        btnBack.setOnClickListener {
            val intent = Intent(this, WelcomeScreen::class.java) //(GeeksforGeeks, 2024)
            startActivity(intent) //(GeeksforGeeks, 2024)
            finish()//(GeeksforGeeks, 2024)
        }
